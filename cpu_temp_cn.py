...
''' + rtext_cmd('##temperature show §a显示cpu温度的详情§r', '显示温度详情', prefix1 + ' show') + '''
''' + rtext_cmd('##temperature start §a启动温度预警循环§r', '启动预警循环', prefix1 + ' start') + '''
''' + rtext_cmd('##temperature stop §a停止温度预警循环§r', '停止预警循环', prefix1 + ' stop') + '''
§b-----------------------------------§r'''
...
class TemperatureChecker:
    ...
    def cal_temp(self, num=0, server: 'CBRInterface' = None, info: 'MessageInfo' = None, reply_msg=""):
        ...
        highest_temp_msg = 'cpu单核心最高温度' + ' : ' + temp_color(temp_cache[0][1])
        reply_msg += temp_msg
        ...
        if num == 0 or num == 3:
            ...
            if tempe > warning_degree:
                ...
                print_msg(
                    ...
                    rtext_cmd(' §e[▷]§r', '点击获得更多资讯', prefix1 + ' show') +
                    '\n' + rtext_cmd('§e====点击获取更多资讯====§r', '点击获取资讯详情', prefix1 + ' show'),
    ...
    def stop(self):
        ...
        self.task.shutdown()
        self.cmd_server.logger.info(system_return + '循环停止')
...
def start_task(server: 'CBRInterface', info: 'MessageInfo'):
    if not task_start:
        ...
        task.open_sche()
        print_msg(system_return + '循环开始了', 1, server, info)
    else:
        print_msg(error + '循环早就开始了', 1, server, info)
...
def stop_task(server: 'CBRInterface', info: 'MessageInfo'):
    if not task_start:
        ...
        task.stop()
        server.reply(info, system_return + '循环已经停止')
